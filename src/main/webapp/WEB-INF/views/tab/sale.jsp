...
<div id="Container">
	...
	<div class="sale-area">
		...
		<div class="common-menu">
			<ul>
				<li
					class="<c:if test="${empty pageMaker.cri.cateId ||pageMaker.cri.cateId eq'1000001'}">on</c:if>">
					<button type="button" data-ref-dispcatno="1000001"
						data-attr="세일^세일카테고리_핫인기세일^전체">전체</button>
				</li>
				<!-- 그 외 -->
				<c:forEach var="cate" items="${categ}">
					<li
						class="<c:if test="${pageMaker.cri.cateId eq cate.cateId }">on</c:if> ">
		...
		<div class="TabsConts on">
			...
			<ul class="cate_prd_list gtm_sale_list">
				<c:forEach var="item" items="${sale_list}">
					<li class="flag">
						<div class="prd_info ">
							<a href="/product-detail?pid=${item.id }" name="Sale_Pop"
								class="prd_thumb goodsList"> <span class="newOyflag time"
								style="display: none;">
							...
							<button
								class="btn_zzim jeem <c:if test="${item.wished eq 'true'}">on</c:if>"
							...
							<p class="prd_btn_area">
								<button class="btn_new_pop goodsList"
									onclick="window.open('/product-detail?pid=${item.id }')">새창보기</button>
							</p>
						</div>
					</li>
				</c:forEach>
